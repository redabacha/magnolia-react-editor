...
export class AbstractArea implements OnInit, AfterViewInit {
   ...
   * Constructor
   */
  constructor(private elementRef: ElementRef, private mgnlCtxService: MagnoliaContextService, private resolver: ComponentFactoryResolver) {
  }
   ...
   */
  ngOnInit() {
    this.mgnlCtxService.getAreaComponents(this.cmsArea).then(data => {
  ...
  loadComponents(current: AbstractArea) {
    current.templates.forEach(function(item: ComponentDirective, index: number, array: ComponentDirective[]){
      ...
      const componentClass = current.mgnlCtxService.getComponentMapping(component['mgnl:template']);
  ...
  loadComponent(componentClassName: string, item: ComponentDirective, component: any) {
    ...
    const componentClass = factories.find((x: any) => x.key === componentClassName);

    // Gets the component factory
    const componentFactory = this.resolver.resolveComponentFactory(<Type<any>> componentClass);

    // Get the view container
    item.viewContainerRef.clear();
    const componentRef = item.viewContainerRef.createComponent(componentFactory);
