...
import pageContent from '../../../mock/page.json';
import templateDefinitions from '../../../mock/template-definition.json';
...
describe('EditableArea component', () => {
  ...
  it('Render EditableArea default', () => {
    ...
    // THEN
    expect(container.innerHTML.includes(cleanString(areaCommentText))).toBe(
      true
    );
  ...
  it('Render nested EditableArea', () => {
    ...
    render(
      <EditorProvider value={state}>
        <div>
          <EditableArea
            content={pageContent.main['0'].nestedArea}
            parentTemplateId={pageContent.main['0']['mgnl:template']}
          />
        </div>
      </EditorProvider>,
      container
